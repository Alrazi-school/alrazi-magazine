...
export function getInitials(n: string) {
  return n.split(' ').slice(0, 2).map((x: string) => x[0]).join('')
}

export function getRoleLabel(r: string) {
  return ({ ADMIN: 'مدير', TEACHER: 'معلم', STUDENT: 'طالب' } as any)[r] || r
}

export function getStatusLabel(s: string) {
  return ({ PUBLISHED: 'منشور', REVIEW: 'قيد المراجعة', DRAFT: 'مسودة', REJECTED: 'مرفوض' } as any)[s] || s
}
